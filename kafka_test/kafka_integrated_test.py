...
global_topic_name = "test20160724"
...
def verify_producer():
    ...
    p.produce(global_topic_name, 'Hello Python!')
    p.produce(global_topic_name, key='Just a key')
    p.produce(global_topic_name, partition=1, value='Strictly for partition 1',
              key='mykey')

    # Produce more messages, now with delivery report callbacks in various forms.
    mydr = MyTestDr()
    p.produce(global_topic_name, value='This one has a dr callback',
              callback=mydr.delivery)
    p.produce(global_topic_name, value='This one has a lambda',
              callback=lambda err, msg: MyTestDr._delivery(err, msg))
    p.produce(global_topic_name, value='This one has neither')

    # Produce even more messages
    for i in range(0, 10):
        p.produce(global_topic_name, value='Message #%d' % i, key=str(i),
                  callback=mydr.delivery)
        p.poll(0)
...
def verify_producer_performance(with_dr_cb=True):
    ...
    for i in range(0, msgcnt):
        try:
            if with_dr_cb:
                p.produce(global_topic_name, value=msg_payload, callback=dr.delivery)
            else:
                p.produce(global_topic_name, value=msg_payload)
...
def verify_consumer():
    ...
    c = confluent_kafka.Consumer(**conf)
    offsets = c.committed(list(map(lambda p: confluent_kafka.TopicPartition(global_topic_name, p), range(0,3))))
    for tp in offsets:
        print(tp)
...
def verify_consumer_performance():
    ...
    c.subscribe([global_topic_name], on_assign=my_on_assign, on_revoke=my_on_revoke)
